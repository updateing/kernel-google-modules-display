...
/**
 * struct s6e3hc4_mode_data - panel mode specific details
 *
...
#define S6E3HC4_TE2_CHANGEABLE 0x31
#define S6E3HC4_TE2_FIXED      0x41
...
static const struct s6e3hc4_mode_data s6e3hc4_mode_120 = {
	.idle_vrefresh = 10,
};
...
static void s6e3hc4_update_te2(struct exynos_panel *ctx)
{
	...
	EXYNOS_DCS_BUF_ADD(ctx, 0xB0, 0x00, 0x38, 0xF2);
	EXYNOS_DCS_BUF_ADD(ctx, 0xF2, 0x0C);
	...
	const struct exynos_panel_mode *pmode, bool enforce)
{
	...
	if (test_bit(FEAT_EARLY_EXIT, changed_feat)) {
		...
		if (test_bit(FEAT_EARLY_EXIT, spanel->feat)) {
			...
			EXYNOS_DCS_BUF_ADD(ctx, 0xB9, 0x07, 0xBA, 0x07, 0xBA, 0x07, 0xBC);
		} else {
			/* Changeable TE */
...
static bool s6e3hc4_set_self_refresh(struct exynos_panel *ctx, bool enable)
{
	...
	const struct s6e3hc4_mode_data *mdata;
	struct s6e3hc4_panel *spanel = to_spanel(ctx);
	bool idle_active;
	...
	if (!mdata->idle_vrefresh) {
		bool ee = test_bit(FEAT_EARLY_EXIT, spanel->feat);
				  ...
				  const struct exynos_panel_mode *pmode)
{
	...
	s6e3hc4_write_display_mode(ctx, &pmode->mode, NULL);
	s6e3hc4_change_frequency(ctx, pmode);
	usleep_range(delay_us, delay_us + 10);
...
static const struct exynos_dsi_cmd s6e3hc4_init_cmds[] = {
	...
	EXYNOS_DSI_CMD_SEQ(0xF2, 0x88, 0x80),		/* SEQ_TSP_SYNC_ON */
...
static int s6e3hc4_enable(struct drm_panel *panel)
{
	...
	s6e3hc4_write_display_mode(ctx, mode, NULL); /* dimming and HBM */
	s6e3hc4_change_frequency(ctx, pmode);
				 ...
				 enum exynos_hbm_mode mode)
{
	...
	if (mode != HBM_OFF) {
		...
		s6e3hc4_write_display_mode(ctx, &pmode->mode, &irc);
	} else {
		irc = false;
...
static void s6e3hc4_mode_set(struct exynos_panel *ctx,
			     const struct exynos_panel_mode *pmode)
{
	const struct s6e3hc4_mode_data *mdata = pmode->priv_data;
