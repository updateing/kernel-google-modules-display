...
{
	scnprintf(mode->name, DRM_DISPLAY_MODE_LEN, "%ux%ux%d",
					       ...
					       const struct exynos_drm_connector_state *state)
{
	...
	if (ctx->current_mode) {
		...
		drm_printf(p, " \tcurrent mode: %ux%u@%d\n", m->hdisplay,
					    ...
					    struct drm_connector_state *conn_state)
{
	...
	if (unlikely(!current_mode)) {
		...
		if (current_mode->hdisplay != target_mode->hdisplay ||
			...
			dev_dbg(ctx->dev, "%s: current %ux%u@%d, target %ux%u@%d, type %d\n",
				__func__, current_mode->hdisplay, current_mode->vdisplay,
				current_vrefresh, target_mode->hdisplay, target_mode->vdisplay,
				target_vrefresh, ctx->mode_in_progress);
				  ...
				  const struct drm_display_mode *adjusted_mode)
{
	...
	dev_dbg(ctx->dev, "changing display mode to %ux%u@%d\n",
